...
output$Indicador.paises.dinamico.ui <- shiny::renderUI({
  ...
  shiny::selectInput("Indicador.series", label = "Indicador", choices = opciones, selected = "B6N", width = '240px')
})
...
"D61" = "cotizaciones_soc_paid (D61)", "D63" = "transfer_sociales (D63)", "B2A3N" = "exc_bruto_exp_rent (B2A3N)", "D7" = "otras_transferencias (D7)")
    ...
    if(input$Alcance == "Países") {
      ...
      tb <- data %>% 
        filter(NUTS == 0, full_name == input$Paises.series2[1], na_item %in% codigos_regiones, unit == unit.series) %>% 
  ...
  if(input$Alcance == "Países") {
     ...
     if(input$Indicador.series %in% c("B5N", "B6N")) {
        tb <- data %>% filter(NUTS==0, na_item==input$Indicador.series, unit==unidad_filtro, full_name %in% input$Paises.series) %>% rename(valores=values)
     } else {
        
        tb <- data %>% filter(NUTS==0, na_item==input$Indicador.series, unit=="MIO_EUR", full_name %in% input$Paises.series) 
...
renderTable({
  ...
  req(nrow(input.processing.series.temporales()[[1]]) > 0)
...
shiny::selectInput(
  "Alcance.comparacion",
  label = "Alcance",
  choices = c("Países", "Regiones"), # <--- CAMBIO: 'Comunidades España' -> 'Regiones'
  selected = "Países"
)

# 2. INPUTS X e Y (DINÁMICOS - Se mantienen igual)
...
# Lógica de opciones para X e Y (Adaptada a 'Regiones')
...
# --- 4. LÓGICA DE SELECCIÓN DE TERRITORIO (REESTRUCTURADA) ---

# CASO PAÍSES: Solo seleccionamos el país objetivo
conditionalPanel(
...
input.processing.comparacion <- reactive({
  ...
  # Barrera de seguridad: Esperar a que los inputs existan
  req(input$X, input$Y, input$Alcance.comparacion)
  
  indicador_x <- input$X
  indicador_y <- input$Y
  
  # --- LÓGICA DE FILTRADO Y PIVOTE ---
  ...
  if(input$Alcance.comparacion == "Países") {
    ...
    tb <- data %>% 
      ...
      # Pivote
      select(full_name, TIME_PERIOD, na_item, values) %>% # Limpieza previa clave
    ...
    req(input$Comunidad.comparacion) # Esperamos al input dinámico de región única
...
input.processing.arima <- reactive({
  ...
  if(input$Indicador.arima %in% c('B5N', 'B6N', 'B7N')) {
    ...
    if(input$Scope == 'Países') {
       req(input$Pais) 
       tb <- data %>% 
         filter(NUTS == 0, na_item == input$Indicador.arima, unit == unidad_filtro) %>% 
    ...
    if(input$Scope == 'Países') {
       req(input$Pais)
       tb <- data %>% filter(NUTS == 0, na_item == input$Indicador.arima, unit == 'MIO_EUR', direct == 'RECV', full_name %in% input$Pais[1:2])
    } else {
       req(input$Regiones.Arima)
  ...
  # CASO 3: Resto de Indicadores MIO_EUR (Normalización Estándar)
    ...
    if(input$Scope == 'Países') {
       req(input$Pais)
       tb <- data %>% filter(NUTS == 0, na_item == input$Indicador.arima, unit == 'MIO_EUR', full_name %in% input$Pais[1:2])
    } else {
       req(input$Regiones.Arima)
