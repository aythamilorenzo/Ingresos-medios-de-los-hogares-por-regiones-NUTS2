...
shiny::selectInput(
 ...
 choices = c("Países", "Provincias España"),
 selected = "Países"
)
...
output$Indicador.arima.dinamico <- shiny::renderUI({
  ...
  if(input$Scope == "Países") {
    ...
    # Lógica para PROVINCIAS ESPAÑA (Unidad Fija: EUR_HAB)
...
conditionalPanel(
  condition = "input.Scope == 'Provincias España'",
  shiny::selectInput(
 "Provincia", 
 label = "Provincias (múltiples)",
    ...
    filter(Región %in% input$Pais[1:2]) 
      ...
      tb <- data %>% 
        ...
        rename(Región = full_name,
               Año = TIME_PERIOD,
               valores = values) %>% 
        filter(Región %in% input$Provincia[1:2])
    } else if(input$Indicador.arima %in% c('D4', 'D62')) {
...
renderPlotly({
  ...
  titulo <- input$Pais[1]
    ...
    ts <- tb %>% 
    select(Año, Región, valores) %>% 
    filter(Región %in% input$Provincia[1]) %>% 
    mutate(Año = as.numeric(Año)) %>% 
    as_tsibble(index = Año, key = Región) %>% 
    fill_gaps()
    
  titulo <- paste(input$Provincia[1], "(€ por habitante)")  
  }
...
renderPlotly({
  ...
  titulo <- input$Pais[2]
    ...
    ts <- tb %>% 
    select(Año, Región, valores) %>% 
    filter(Región %in% input$Provincia[2]) %>% 
    mutate(Año = as.numeric(Año)) %>% 
    as_tsibble(index = Año, key = Región) %>% 
    fill_gaps()
  titulo <- paste(input$Provincia[2], "(€ por habitante)")  
  }
...
"D61" = "cotizaciones_sociales (D61)", "D63" = "transfer_sociales (D63)", "B2A3N" = "exc_bruto_exp_rent (B2A3N)", "D7" = "otras_transferencias (D7)")
  ...
  tb <- data %>% 
    ...
    left_join(poblacion %>% select(-freq, -unit), by = c("geo", "TIME_PERIOD")) %>% 
    mutate(per.capita = (values * 1000000) / (poblacion * 1000)) %>% 
    select(-values) %>% 
    rename(values = per.capita) %>% 
