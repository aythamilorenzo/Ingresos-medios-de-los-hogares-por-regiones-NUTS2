...
opciones.series.paises <- data %>% 
  filter((NUTS == 0) | (full_name == 'Media UE') | (full_name == 'Canarias' & NUTS == 2)) %>% 
  pull(full_name) %>% 
  unique()
...
conditionalPanel(
  ...
  shiny::selectInput(
    ...
    choices = opciones.series.paises,
    selected = c("Spain", "Portugal", "France"),
    multiple = TRUE
  ),
  
  shiny::checkboxInput("SelectAll.Paises", "Seleccionar todos los países", value = FALSE)
)
...
conditionalPanel(
  ...
  shiny::selectInput(
    "Pais.Padre",  
    label = "País:",
    choices = (data %>% filter(NUTS == 0)) %>% pull(full_name) %>% unique(),
    selected = "Spain"
  ),
...
shiny::observeEvent(input$SelectAll.Paises, {
  if (input$SelectAll.Paises) {
    shiny::updateSelectInput(session, "Paises.series", selected = opciones.series.paises)
  } else {
    shiny::updateSelectInput(session, "Paises.series", selected = c("Spain", "Portugal", "France", "Media UE", "Canarias", "Germany"))
  }
...
output$UI.Selector.Regiones <- shiny::renderUI({
  ...
  opciones_regionales <- data %>%
    filter((NUTS == 2 & substr(geo, 1, 2) == codigo_pais) | (full_name == 'Media UE')) %>%
  ...
  if (!is.null(input$Indicador.series) && input$Indicador.series == 'Todos los indicadores') {
     ...
     shiny::selectInput("Regiones.series", label = "Región", choices = opciones_regionales, selected = opciones_regionales[1])
     
  } else {
     

     list(
       shiny::selectInput("Regiones.series", label = "Regiones", 
                          choices = opciones_regionales, 
                          selected = opciones_regionales[1:min(4, length(opciones_regionales))], 
                          multiple = TRUE),
       
       shiny::checkboxInput("SelectAll.Regiones", "Seleccionar todas las regiones", value = FALSE)
...
shiny::observeEvent(input$SelectAll.Regiones, {
  
  req(input$Pais.Padre) 
  

  codigo_pais <- data %>% filter(NUTS == 0, full_name == input$Pais.Padre) %>% pull(geo) %>% unique()
  opciones_regionales <- data %>% filter((NUTS == 2 & substr(geo, 1, 2) == codigo_pais) | (full_name == 'Media UE')) %>% pull(full_name) %>% unique() %>% sort()
  
  if (input$SelectAll.Regiones) {
    
    shiny::updateSelectInput(session, "Regiones.series", selected = opciones_regionales)
  } else {
    
    shiny::updateSelectInput(session, "Regiones.series", selected = opciones_regionales[1:min(4, length(opciones_regionales))])
  }
})
```
...
"D61" = "cotizaciones_soc_paid (D61)", "D63" = "transfer_sociales (D63)", "B2A3N" = "exc_bruto_exp_rent (B2A3N)", "D7" = "otras_transferencias (D7)")
    ...
    if(input$Alcance == "Países") {
      ...
      tb <- data %>% 
        filter(((NUTS == 0) | (full_name == 'Media UE') | (full_name == 'Canarias' & NUTS == 2)), full_name == input$Paises.series2[1], na_item %in% codigos_regiones, unit == unit.series) %>% 
      ...
      tb <- data %>% 
        filter((NUTS == 2 | full_name == 'Media UE'), full_name == input$Regiones.series[1], na_item %in% codigos_regiones, unit == unit.series) %>% 
        filter(!(na_item %in% c("D4", "D62")) | direct == "RECV") %>% 
        filter(!(na_item == "D61") | direct == "PAID") %>%
        left_join(poblacion, by = c("geo", "TIME_PERIOD")) %>% 
        mutate(valores = (values * 1000000) / (poblacion * 1000)) %>% 
  ...
  if(input$Alcance == "Países") {
     ...
     if(input$Indicador.series %in% c("B5N", "B6N")) {
        tb <- data %>% filter(((NUTS == 0) | (full_name == 'Media UE') | (full_name == 'Canarias' & NUTS == 2)), na_item==input$Indicador.series, unit==unidad_filtro, full_name %in% input$Paises.series) %>% rename(valores=values)
     } else {
        
        tb <- data %>% filter(((NUTS == 0) | (full_name == 'Media UE') | (full_name == 'Canarias' & NUTS == 2)), na_item==input$Indicador.series, unit=="MIO_EUR", full_name %in% input$Paises.series) 
    ...
    if(input$Indicador.series %in% c("B5N", "B6N", "B7N")) {
      tb <- data %>% filter((NUTS == 2 | full_name == 'Media UE'), na_item==input$Indicador.series, unit=="EUR_HAB", full_name %in% input$Regiones.series) %>% rename(valores=values)
    } else {
      tb <- data %>% filter((NUTS == 2 | full_name == 'Media UE'), na_item==input$Indicador.series, unit=="MIO_EUR", full_name %in% input$Regiones.series)
      if(input$Indicador.series %in% c("D4", "D62")) tb <- tb %>% filter(direct=="RECV")
      tb <- tb %>% left_join(poblacion, by=c("geo","TIME_PERIOD")) %>% mutate(valores=(values*1000000)/(poblacion*1000))
    }
...
conditionalPanel(
  ...
  shiny::selectInput(
    "Pais", 
    label = "Países (Máx 2)",
    choices = data %>% filter((NUTS == 0 | full_name == 'Media UE') | (full_name == 'Canarias' & NUTS == 2)) %>% pull(full_name) %>% unique(),
...
input.processing.arima <- reactive({
  ...
  if(input$Indicador.arima %in% c('B5N', 'B6N', 'B7N')) {
    ...
    if(input$Scope == 'Países') {
       req(input$Pais) 
       tb <- data %>% 
         filter(((NUTS == 0 | full_name == 'Media UE') | (full_name == 'Canarias' & NUTS == 2)), na_item == input$Indicador.arima, unit == unidad_filtro) %>% 
         filter(full_name %in% input$Pais[1:2]) 
    } else {
       req(input$Regiones.Arima) 
       tb <- data %>% 
         filter((NUTS == 2 | full_name == 'Media UE'), na_item == input$Indicador.arima, unit == 'EUR_HAB') %>% 
    ...
    if(input$Scope == 'Países') {
       req(input$Pais)
       tb <- data %>% filter(((NUTS == 0 | full_name == 'Media UE') | (full_name == 'Canarias' & NUTS == 2)), na_item == input$Indicador.arima, unit == 'MIO_EUR', direct == 'RECV', full_name %in% input$Pais[1:2])
    } else {
       req(input$Regiones.Arima)
       tb <- data %>% filter((NUTS == 2 | full_name == 'Media UE'), na_item == input$Indicador.arima, unit == 'MIO_EUR', direct == 'RECV', full_name %in% input$Regiones.Arima[1:2])
    }
  ...
  # CASO 3: Resto de Indicadores MIO_EUR (Normalización Estándar)
    ...
    if(input$Scope == 'Países') {
       req(input$Pais)
       tb <- data %>% filter(((NUTS == 0 | full_name == 'Media UE') | (full_name == 'Canarias' & NUTS == 2)), na_item == input$Indicador.arima, unit == 'MIO_EUR', full_name %in% input$Pais[1:2])
    } else {
       req(input$Regiones.Arima)
       tb <- data %>% filter((NUTS == 2 | full_name == 'Media UE'), na_item == input$Indicador.arima, unit == 'MIO_EUR', full_name %in% input$Regiones.Arima[1:2])
    }
