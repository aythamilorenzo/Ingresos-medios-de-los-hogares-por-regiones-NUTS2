...
library(ggplot2)
library(tidyverse)
library(knitr)    # Quitar
library(eurostat) # Quitar
source("utilidades.R")
...
input.proccesing.tboriginal <- reactive({
...
shiny::renderTable({
    ...
    tb.original <- input.proccesing.tboriginal()
...
```{r}
input.proccesing.tbpca <- reactive({
...
```{r}
shiny::uiOutput("controles_ejes_pca")

output$controles_ejes_pca <- shiny::renderUI({
  

  req(input.processing.modelopca())
  
  pca <- input.processing.modelopca()
  num_componentes <- length(pca$sdev)
  ...
  tags$div(
    ...
    tags$div(
      ...
      shiny::numericInput(
        ...
        value = 1,
        min = 1,
        max = length(pca$sdev), # <--- ¡AQUÍ ESTÁ LA MAGIA!
        step = 1,
        width = "80px"
      )
    ),
    ...
    tags$div(
      ...
      shiny::numericInput(
        ...
        value = 2,
        min = 1,
        max = length(pca$sdev), # <--- ¡AQUÍ TAMBIÉN!
        step = 1,
        width = "80px"
      )
    )
