...
data.top10.plot <- data.top10.plot %>% 
  mutate(
...
data.top10.plot <- data.top10.plot %>% 
  ...
  expand_limits(y = max(data.top10$values) * 1.1)

ggplotly(data.top10.plot, tooltip = "text")
```
...
data.spain.NUTS0.disp.plot <- data.spain.NUTS0.disp %>% 
  ggplot(aes(x = TIME_PERIOD, 
             y = PPS_EU27_2020_HAB, 
             group = full_name, 
             color = full_name,
             text = paste("Año: ", TIME_PERIOD,
                          "<br>Región: ", full_name,
                          "<br>Valores: ", PPS_EU27_2020_HAB, "PPS"))) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(
...
ggplotly(data.spain.NUTS0.disp.plot, tooltip = "text")
```
...
Por último para tener una visión general, veamos unos mapas coropléticos con los valores sobre la renta neta en el último año de las observaciones (2022). Haremos mapas tanto a nivel de NUTS 0 como NUTS 2, para ello cargamos la información necesaria.
...
mapa.NUTS0.renta.join <- geoj.tb %>% 
  left_join(mapa.NUTS0.renta, by = c("ADMIN" = "full_name"))

etiquetas <- paste( "<strong>", geoj.tb$ADMIN,
                   "</strong><br><strong>Valor: </strong>", 
                   mapa.NUTS0.renta.join$values, "<strong>PPS/HAB</strong>") %>% 
...
etiquetas <- paste( "<strong>", geoj2.tb$NUTS_NAME,
                   "</strong><br><strong>Valor: </strong>", 
                   mapa.NUTS2.renta.join$values, "<strong>PPS/HAB</strong>") %>% 
  lapply(htmltools::HTML)

leyenda <- "Renta neta<br>disponible"
MapaCoroplético2(geoj2, mapa.NUTS2.renta.join$values, etiquetas, leyenda)
```
