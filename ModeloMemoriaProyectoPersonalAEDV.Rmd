...
datatop10.imp.plot %>%
  ...
  scale_fill_gradient(low = "white", 
                      high = "steelblue") +
geom_text(aes(label = round(imp.per.capita,1)), color = "black") +
...
Ahora, separemos España en 6 zonas (incluyendo Canarias) y veamos que proporción de la renta total se lleva cada parte. Hay que tener en cuenta que no estamos considerando la población que hay en cada zona
...
data.spain.pie %>%
  ggplot(aes(x = "",
             ...
             fill = full_name)) +
  geom_col() +
  geom_text(aes(x=1.,label = porcentaje),size=6, position = position_stack(vjust = 0.5)) +
  coord_polar(theta = "y") +
...
En España, el PPS por habitante es mayor que el ingreso nominal en todas las regiones, lo que indica que los precios locales son más bajos que la media de la UE, y por tanto el poder adquisitivo real de los hogares españoles es superior al que sugieren los ingresos nominales. La región Noreste muestra la mayor renta y poder adquisitivo, mientras que el Sur es la más desfavorecida

```{r}

data.spain.disp <- data %>% 
  filter(substr(geo,1,2) == 'ES' & NUTS == 1 & unit %in% c('PPS_EU27_2020_HAB', 'EUR_HAB') & !full_name %in% c('Comunidad de Madrid', 'Canarias') & TIME_PERIOD == max(TIME_PERIOD) & na_item == 'B6N') %>% 
  select(full_name, unit, values, na_item) %>% 
  pivot_wider(names_from = unit, values_from = values)

data.spain.disp %>% 
  ggplot(aes(x = PPS_EU27_2020_HAB, 
             y = EUR_HAB,
             color = full_name)) +
  geom_point(alpha = 0.7) +
  geom_text(aes(label = full_name), hjust = 0.5, vjust = -0.5, size = 3) +
  scale_color_brewer(palette = "Set2") +
  labs(
    title = "España: Renta neta en PPS vs Renta neta en EUR/HAB",
    subtitle = "Por región",
    x = "PPS per cápita",
    y = "Ingreso medio por habitante",
  ) +
  theme_minimal() +
