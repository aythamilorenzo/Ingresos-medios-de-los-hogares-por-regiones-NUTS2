...
Una de las principales motivaciones que me ha llevado a elegir un dataset de este tema es mi deseo de vivir fuera de España, sobretodo por temas laborales entre otros y por calidad de vida. Yo considero que los aspectos económicos son un factor muy importante a tener en cuenta para esto, el poder comparar a España con otros países o incluso poder comparar España internamente por indicadores económicos diferentes es algo que me ha impulsado a elegir este dataset.
...
data.ahorro.es.plot <- data.ahorro.es %>% 
  ...
  labs(
    title = "Ahorro de los hogares por región Española (2022)",
    x = NULL,
...
data.top10PPS.plot <- rbind(data.top.renta.PPS, data.canarias.mediaue.renta.PPS) %>% arrange(desc(values))
...
data.top10PPS.plot <- data.top10PPS.plot %>% 
  ...
  labs(x = "", y = "Renta neta per cápita (PPS)") +
  coord_flip() +
  theme_bw(base_size = 14) +
  theme(legend.position = "none") +
  labs(title = "Evolución de la renta neta per cápita | Año: {current_frame}") +
  transition_manual(TIME_PERIOD)
  
animate(data.top10PPS.plot, fps = 2, end_pause = 10)
```
...
Vamos a ver una gráfica por países que nos mostrará, el ingreso primario neto, la renta neta y la diferencia entre ambos, con esto podremos ver cuanto se descuenta en cada país del salario bruto.

```{r}
data.diff.paises <- data %>% 
  filter(((NUTS == 0) | (full_name == 'Canarias' & NUTS == 2)) & na_item %in% c("B5N", "B6N", "D62"), unit == 'MIO_EUR', TIME_PERIOD == max(TIME_PERIOD),
         (!na_item == "D62" | direct == "RECV"), !full_name %in% c('Slovakia',"Slovenia", "Czechia")) %>%
  select(-direct) %>% 
  left_join(poblacion, by = c('geo', 'TIME_PERIOD')) %>% 
  ...
  mutate(Diferencia = B5N - (B6N - D62)) %>% 
  rename(Región = full_name,
         Año = TIME_PERIOD,
         `Ingreso primario neto` = B5N,
         `Renta neta` = B6N,
         `Prestaciones sociales recibidas` = D62) %>% 
  select(Región, Año, `Ingreso primario neto`, `Renta neta`,`Prestaciones sociales recibidas`, Diferencia)

p <- data.diff.paises %>% 
  rename(`Ingreso primario` = `Ingreso primario neto`,
         Prest.soc.recib = `Prestaciones sociales recibidas`) %>% 
  pivot_longer(`Ingreso primario`:`Diferencia`, names_to = 'Indicador', values_to = 'valores') %>% 
  ggplot(aes(x = Indicador,
             y = valores,
             fill = Indicador,
             text = paste("Región:", Región,
                          "<br>",Indicador,":", round(valores,1), "€/HAB"))) +
    geom_bar(stat = 'identity') + 
    facet_wrap(~ Región) +
    theme_bw(base_size = 14) +
     theme(
...
ggplot(aes(x = B6N,
           ...
           text = paste("Región: ", full_name,
                        "<br>Renta neta: ", B6N, "€",
                        "<br>Ahorro neto: ", B7N, "€"))) +
  geom_point(colour = "steelblue", aes(size = 3, frame = Año)) +
  labs(
    title = "Relación entre renta y ahorro por comunidades de España",
