...
data.imp.comunidades.plot <- data.imp.comunidades %>%
  ...
  geom_text(aes(label = round(imp.per.capita,1)), color = "black", size = 3.5)+
...
data.imp.plot <- data.imp %>%
  ...
  scale_fill_gradient(low = "white", high = "steelblue") + 
      geom_text(aes(label = round(Impuestos.pagados,1)), color = "black", size = 3.5) +
...
leyenda <- "Renta neta<br>disponible<br>(2022)<br>
(PPS/Hab)"
...
leyenda <- "Inversión en<br>activos fijos<br>(2022)<br>
(€/Hab)"
MapaCoropléticoMemoria(geoj2_filtrado, datos_filtrados$values, etiquetas, leyenda)
```
<br>
