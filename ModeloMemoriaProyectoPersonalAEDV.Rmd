...
Haciendo referencia a algunos de los modelos matemáticos utilizados en el análisis de modelado, vamos a comentar brevemente según los resultados obtenidos, si estos modelos pueden ser una **representación fiable de la realidad.**

En las **predicciones ARIMA**, para el caso de provincias de España, vemos intervalos de confianza que no son realmente grandes, podemos decir que el modelo ha tenido éxito y se ha ajustado bien proporcionando una **predicción fiable.**

Con respecto a **España** como país, los intervalos para la **renta neta** y para los **impuestos pagados** son bastante más grandes. Esto no quiere decir que la predicción no sea fiable, simplemente hay más **incertidumbre** y probablemente la evolución de estas dos en el futuro venga dada por factores externos aunque en el intervalo de confianza es prácticamente ascendente.

Por otro lado, tenemos el **análisis de componentes principales** y la **matriz de correlación** donde vemos que las variables poseen una **fuerte correlación en general** entre ellas salvo casos puntuales. Vemos que, con una única componente se puede explicar más del **60%** de la variación de los datos, siendo que con 3 de estas componentes explicamos más del **80%**, esto puede ser un indicador de que no es necesario tener tantas variables para explicar muchos de los hechos que hemos contemplado en el análisis de este proyecto.

Por último, tenemos un diagrama de dispersión usando un **modelo de regresión lineal** y transformando la variable del eje y mediante **yeojohnson** en el que se aprecia una fuerte relación entre la **renta neta per cápita** y la **presión fiscal**, el modelo presenta un coeficiente **R2 de 0.88** que confirma lo que venimos comentando.  
...
También hay un input que al marcarlo, automáticamente se seleccionan **todos los países o regiones de un país**. Se permite en países seleccionar a **Canarias** para poder ver el **aglomerado de la Unión Europea, España y Canarias** en una misma pestaña. Por otro lado cuando se van a seleccionar las regiones de un país, también se permite escoger el **aglomerado de la Unión Europea** para compararlo con las distintas regiones directamente.
