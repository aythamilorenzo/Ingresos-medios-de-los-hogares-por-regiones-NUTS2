...
```{r}
datatsrenta.spain.arima %>%
  forecast(h = 5) %>%
  autoplot(datatsrenta.spain) +
  theme_bw(base_size = 14)
...
```{r}
datatsahorro.spain.arima %>% 
  forecast(h = '5 years') %>% 
  autoplot(datatsahorro.spain) + 
  theme_bw(base_size = 14)
```
...
datatsactivos.spain %>% 
  ...
  theme_bw(base_size = 14) +
    labs(x = "Año", y = "Ingresos",
       title = "Evolución de ingresos a partir de activos en España",
       subtitle = "Valores en € por habitante") 
```
...
```{r}
datatsactivos.spain.arima %>% 
  forecast(h = "5 years") %>% 
  autoplot(datatsactivos.spain) +
  theme_bw(base_size = 14)
```

```{r}
ingresos.a.partir.de.activos <- datatsactivos.spain$valores
datatsactivos.spain.STL <- datatsactivos.spain %>% 
  model(STL(ingresos.a.partir.de.activos, robust = TRUE)) %>% 
  components()
```

```{r}
datatsactivos.spain.STL %>% 
  autoplot() +
  theme_bw(base_size = 14)
```
