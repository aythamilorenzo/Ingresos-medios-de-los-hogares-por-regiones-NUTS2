...
data.es <- data %>%
  ...
  rename(Año = TIME_PERIOD,
         valores = values) %>%
  mutate(
    grupo_color = ifelse(full_name == "Canarias", "Canarias", "Otras Regiones"),
...
data.ahorro.es <- data %>%
  ...
  arrange(values) %>%
  mutate(
    grupo_color = ifelse(full_name == "Canarias", "Canarias", "Otros")
  )
...
data.ahorro.es.plot <- data.ahorro.es %>% 
  ggplot(aes(x = reorder(full_name, values), 
             y = values,
             group = full_name,
             fill = grupo_color, # Usamos 'fill' para el color de las barras
             text = paste("Región: ", full_name,
                          "<br>Valores: ", values, "€"))) +
  geom_col() + # Quitamos el color fijo
  scale_fill_manual(values = c("Canarias" = "red", "Otros" = "#0072B2")) + # Asignamos el rojo a Canarias
  coord_flip() +
  theme_minimal() +
  theme(
...
Ahora vamos a ver un top 10 de los países con mejor renta neta por habitante medido en unidades PPS que nos permitirán hacer una comparación justa entre países.
...
data.top10PPS.plot <- data.top10PPS.plot %>% 
  mutate(
  ...
  labs(
    ...
    subtitle = paste("Último año disponible: " , max(data.top10PPS$TIME_PERIOD), "(Valores tomados en PPS)"),
    x = NULL,
  ...
  expand_limits(y = max(data.top10PPS$values) * 1.1) 

ggplotly(data.top10PPS.plot, tooltip = "text")
```
...
Vamos a observar con un mapa de calor los impuestos en millones de euros que ha ido pagando cada país (como conjunto) desde 2016 hasta 2022. Para este caso, dado que el dataset no nos ofrece una opción para ver el número de impuestos per cápita, utilizaremos el dataset secundario de población que hemos añadido anteriormente y haremos las operaciones necesarias para normalizar estos valores transformándolos a per cápita. Hay que tener en cuenta que los valores de los impuestos están en millones de euros y los de la población en miles de personas.
...
data.imp.plot <- data.imp %>%
  ggplot(aes(x = TIME_PERIOD,
             ...
             text = paste("Año: ", TIME_PERIOD,
                          "<br>Región: ", full_name,
                          "<br>Valores: ", imp.per.capita))) +
  geom_tile(color = "grey80") +
  scale_fill_gradient(low = "white", 
                      high = "steelblue") +
  labs(title="Impuestos sobre renta y patrimonio",
       subtitle = "2016-2022",
       x="Año",
       y="País",
...
ggplotly(data.spain.box.plot)
```

Por último para tener una visión general, veamos unos mapas coropléticos con los valores sobre varios indicadores económicos en el último año de las observaciones (2022). Haremos mapas tanto a nivel de NUTS 0 como NUTS 2, para ello cargamos la información necesaria.
...
Empecemos por una visión general de la renta neta.

```{r}
mapa.NUTS0.renta <- data %>% 
  filter(NUTS == 0, na_item == "B6N", unit == "PPS_EU27_2020_HAB", TIME_PERIOD == max(TIME_PERIOD))  

mapa.NUTS0.renta.join <- geoj.tb %>% 
  left_join(mapa.NUTS0.renta, by = "geo")

etiquetas <- paste( "<strong>", geoj.tb$NUTS_NAME,
                   "</strong><br><strong>Valor: </strong>", 
                   round(mapa.NUTS0.renta.join$values, 1), "<strong>PPS/HAB</strong>") %>% 
  lapply(htmltools::HTML)

leyenda <- "Renta neta<br>disponible"
MapaCoroplético(geoj, mapa.NUTS0.renta.join$values, etiquetas, leyenda)
```

Veamos ahora la renta pero en este caso para regiones NUTS2.
...
etiquetas <- paste( "<strong>", geoj2.tb$NUTS_NAME,
                   "</strong><br><strong>Valor: </strong>", 
                   round(mapa.NUTS2.renta.join$values, 1), "<strong>PPS/HAB</strong>") %>% 
  lapply(htmltools::HTML)

leyenda <- "Renta neta<br>disponible"
MapaCoroplético(geoj2, mapa.NUTS2.renta.join$values, etiquetas, leyenda)
```


```{r}
mapa.NUTS0.ahorro <- data %>% 
  filter(NUTS == 0, na_item == "B5N", unit == "PPS_EU27_2020_HAB", TIME_PERIOD == max(TIME_PERIOD))  

mapa.NUTS0.ahorro.join <- geoj.tb %>% 
  left_join(mapa.NUTS0.ahorro, by = "geo")

etiquetas <- paste( "<strong>", geoj.tb$NUTS_NAME,
                   "</strong><br><strong>Valor: </strong>", 
                   round(mapa.NUTS0.ahorro.join$values, 1), "<strong>PPS/HAB</strong>") %>% 
  lapply(htmltools::HTML)

leyenda <- "Ingreso<br>primario<br>neto"
MapaCoroplético(geoj, mapa.NUTS0.ahorro.join$values, etiquetas, leyenda)
```

Veamos un mapa por NUTS 2 sobre el ingreso primario neto (antes de impuestos) en 2022

```{r}
mapa.NUTS2.ahorro <- data %>% 
  filter(NUTS == 2, na_item == "B5N", unit == "PPS_EU27_2020_HAB", TIME_PERIOD == max(TIME_PERIOD))  

mapa.NUTS2.ahorro.join <- geoj2.tb %>% 
  left_join(mapa.NUTS2.ahorro, by = "geo")

etiquetas <- paste( "<strong>", geoj2.tb$NUTS_NAME,
                   "</strong><br><strong>Valor: </strong>", 
                   round(mapa.NUTS2.ahorro.join$values, 1), "<strong>PPS/HAB</strong>") %>% 
  lapply(htmltools::HTML)

leyenda <- "Ingreso<br>primario<br>neto"
MapaCoroplético(geoj2, mapa.NUTS2.ahorro.join$values, etiquetas, leyenda)
```


```{r}
mapa.NUTS0.inversionactivos <- data %>% 
  filter(NUTS == 0, na_item == "P51C", unit == "MIO_EUR", TIME_PERIOD == max(TIME_PERIOD)) %>% 
  left_join(poblacion, by = c('geo', 'TIME_PERIOD')) %>% 
  mutate(per.capita = (values * 1000000) / (poblacion * 1000)) %>% 
  select(-values) %>% 
  rename(values = per.capita)

mapa.NUTS0.inversionactivos.join <- geoj.tb %>% 
  left_join(mapa.NUTS0.inversionactivos, by = "geo")

etiquetas <- paste( "<strong>", geoj.tb$NUTS_NAME,
                   "</strong><br><strong>Valor: </strong>", 
                   round(mapa.NUTS0.inversionactivos.join$values, 1), "<strong>PPS/HAB</strong>") %>% 
  lapply(htmltools::HTML)

leyenda <- "Inversión en<br>activos fijos"
MapaCoroplético(geoj, mapa.NUTS0.inversionactivos.join$values, etiquetas, leyenda)
```
...
etiquetas <- paste( "<strong>", geoj2.tb$NUTS_NAME,
                   "</strong><br><strong>Valor: </strong>", 
                   round(mapa.NUTS2.inversionactivos.join$values, 1), "<strong>PPS/HAB</strong>") %>% 
  lapply(htmltools::HTML)

leyenda <- "Inversión en<br>activos fijos"
MapaCoroplético(geoj2, mapa.NUTS2.inversionactivos.join$values, etiquetas, leyenda)
```
...
```{r}
datatsrenta.spain.plot <- datatsrenta.spain %>%
  autoplot(size = 0.7) +
  theme_bw(base_size = 14) +
  labs(x = "Año", y = "Renta", title = "Evolución de la Renta Neta por habitante en España", subtitle = "Valores tomados en € por habitante")

ggplotly(datatsrenta.spain.plot)
```
...
datatsrenta.spain.titulo <- "Predicción ARIMA de la evolución de la Renta Neta en España"
...
A continuación vamos a ver como han evolucionado los impuestos per cápita en España a lo largo de los años, ya que no tenemos datos por habitante, tendremos que normalizar las observaciones medidas en `MIO_EUR` diviendo dichos valores por la población, filtraremos a partir del año 2002 ya que fue cuando se introdujo el euro a España.
...
```{r}
datatsimp.spain.plot <- datatsimp.spain %>%
  autoplot(size = 0.7) +
  theme_bw(base_size = 14) +
  labs(x = "Año", y = "Impuestos pagados en € por habitante",
       title = "Evolución de Impuestos sobre Renta y Patrimonio en España",
       subtitle = "Valores per cápita")

ggplotly(datatsimp.spain.plot)
```
...
datatsimp.spain.arima.titulo <- "Predicción ARIMA de la evolución de Impuestos sobre Renta y Patrimonio en España"
...
Vamos a continuar viendo la evolución y predicción de distintas variables económicas de interés en España.

```{r}
datatsahorro.spain <- data %>% 
  filter(na_item == 'B7N' & geo == 'ES' & unit == 'EUR_HAB' & TIME_PERIOD > 2001) %>% 
  arrange(TIME_PERIOD, values) %>% 
  rename(Año = TIME_PERIOD,
         valores = values) %>% 
  select(Año, valores) %>% 
...
Su evolución es parecida a la de la renta neta y tiene todo el sentido ya que una mayor renta debería estar directamente relacionada con una mayor capacidad de ahorro.
```{r}
datatsahorro.spain.plot <- datatsahorro.spain %>%
  autoplot(size = 0.7) +
  theme_bw(base_size = 14) +
  labs(x = "Año", y = "Ahorro neto en € por habitante",
       title = "Evolución del Ahorro Neto en España",
       subtitle = "Valores per cápita")

ggplotly(datatsahorro.spain.plot)
```

Hagamos una predicción a 5 años con ARIMA.

```{r message=FALSE, warning=FALSE, results = 'hide'}
datatsahorro.spain.arima <- datatsahorro.spain %>% 
  model(ARIMA()) %>% 
  report(fit)
```

```{r}
datatsahorro.spain.arima.pred <- datatsahorro.spain.arima %>% 
  forecast(h = '5 years') 

datatsahorro.spain.arima.titulo <- "Predicción ARIMA del Ahorro Neto en España"
  
GraficoDinamicoArima95CI(datatsahorro.spain, "Año", "valores", datatsahorro.spain.arima.pred, datatsahorro.spain.arima.titulo)
```

```{r}
ahorro.neto <- datatsahorro.spain$valores
datatsahorro.spain.STL <- datatsahorro.spain %>% 
  model(STL(ahorro.neto, robust = TRUE)) %>% 
  components()
```

```{r}
datatsahorro.spain.STL.plot <- datatsahorro.spain.STL %>% 
  autoplot() +
  theme_bw(base_size = 14)

ggplotly(datatsahorro.spain.STL.plot)
```



Otro factor interesante puede ser, los ingresos que los españoles reciben a partir de la propiedad de activos, ya que España por lo general se considera un país con una pobre educación financiera. Para ello, de nuevo tenemos que normalizar los valores por población
```{r}
datatsactivos.spain <- data %>% 
  filter(na_item == 'D4', geo == 'ES', TIME_PERIOD > 2001, direct == 'RECV', unit == "MIO_EUR") %>% 
...
```{r}
datatsactivos.spain.plot <- datatsactivos.spain %>% 
  autoplot(size = 0.7) + 
  theme_bw(base_size = 14) +
    labs(x = "Año", y = "Ingresos",
       title = "Evolución de Ingresos a partir de Activos Fijos en España",
       subtitle = "Valores en € por habitante")

ggplotly(datatsactivos.spain.plot)
```

```{r message=FALSE, warning=FALSE, results = 'hide'}
datatsactivos.spain.arima <- datatsactivos.spain %>% 
  model(ARIMA()) %>% 
  report(fit)
```

```{r}
datatsactivos.spain.arima.pred <- datatsactivos.spain.arima %>% 
  forecast(h = "5 years") 

datatsactivos.spain.arima.titulo <- "Predicción ARIMA de los Ingresos a partir de Activos Fijos en España"
  
GraficoDinamicoArima95CI(datatsactivos.spain, "Año", "valores", datatsactivos.spain.arima.pred, datatsactivos.spain.arima.titulo)
```

```{r}
ingresos <- datatsactivos.spain$valores
datatsactivos.spain.STL <- datatsactivos.spain %>% 
  model(STL(ingresos, robust = TRUE)) %>% 
  components()
```

```{r}
datatsactivos.spain.STL.plot <- datatsactivos.spain.STL %>% 
  autoplot() +
  theme_bw(base_size = 14)

ggplotly(datatsactivos.spain.STL.plot)
```

Ahora, nos centraremos en observar la evolución de España junto con la de otros países,
para ello esta vez usaremos datos PPS.

```{r}
datats.evolucion.renta <- data %>% 
  filter(NUTS == 0 & na_item == 'B6N' & unit == 'PPS_EU27_2020_HAB') %>% 
...
```{r}
datats.evolucion.renta %>% 
  hchart("line", hcaes(x = Año, y = valores, group = Paises)) %>% 
  hc_title(text = "Evolución de la Renta Neta por País") %>% 
  hc_subtitle(text = "Valores tomados en unidades PPS per cápita")

```


Veamos la evolución del ahorro, hay menos países que tienen datos acerca de esto pero podemos hacer una comparativa igualmente.

```{r}
datats.evolucion.ahorro <- data %>% 
  filter(NUTS == 0 & na_item == "B7N" & unit == "PPS_EU27_2020_HAB") %>% 
  select(full_name, TIME_PERIOD, values) %>% 
  rename(Paises = full_name,
         Año = TIME_PERIOD,
         valores = values) %>%
...
```{r}
datats.evolucion.ahorro %>% 
  hchart("line", hcaes(x = Año, y = valores, group = Paises)) %>% 
  hc_title(text = "Evolución del Ahorro Neto por País") %>% 
  hc_subtitle(text = "Valores tomados en unidades PPS per cápita")
```

Por último veremos la evolución del ingreso primario neto.

```{r}
datats.evolucion.ingresoprimario <- data %>% 
  filter(NUTS == 0 & na_item == "B5N" & unit == "PPS_EU27_2020_HAB") %>% 
  select(full_name, TIME_PERIOD, values) %>% 
  rename(Paises = full_name,
         Año = TIME_PERIOD,
         valores = values) %>% 
...
datats.evolucion.ingresoprimario %>% 
  hchart("line", hcaes(x = Año, y = valores, group = Paises)) %>%
  hc_title(text = "Evolución del Ingreso Primario Neto por País") %>% 
  hc_subtitle(text = "Valores tomados en unidades PPS per cápita")
